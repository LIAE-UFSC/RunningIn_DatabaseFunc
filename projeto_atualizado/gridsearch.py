...
def busca_grade_completa(
    input_csv=['dataset_massflow.csv'],  # Agora aceita lista de datasets
    ...
    for combo in itertools.product(*parametros_variados.values()):
        ...
        # Restrição 3: n_amostras deve ser menor ou igual a latent_dim
    ...
    for i, params in enumerate(combinacoes, 1):
        ...
        # Lista para armazenar todos os datasets janelados
        todos_datasets_janelados = []
        
        # Processamento de cada dataset individualmente até o janelamento
        for dataset_path in input_csv:
            ...
            todos_datasets_janelados.append(df_reorg)
        
        # Concatenar todos os datasets janelados
        df_concatenado = pd.concat(todos_datasets_janelados, ignore_index=True)
        
        # Balanceamento sem salvar arquivo
        df_balanceado = balancear_csv_por_undersampling(
            df_dados=df_concatenado,
            output_csv=None,
            embaralhar=False
        )
...
if __name__ == "__main__":
    resultados = busca_grade_completa(
        input_csv=['dataset_A1_01_07.csv', 'dataset_A2_02_10.csv','dataset_A1_01_07.csv', 'dataset_A1_01_07.csv' ],  # Agora pode passar múltiplos datasets
        lista_time_ranges=[[(0, 18000, 0), (54000, 100000, 1)]],
        lista_grey_zones=[(18000, 54000)],
        lista_n_amostras=[8, 16],
